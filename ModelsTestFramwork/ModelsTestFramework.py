...
class RepoInitSummer():
      def __init__(self, repo):
         ...
         print("This is Repo Init!")
         pid = os.getpid() 
         cmd='''ps aux | grep python | grep -v %s | awk '{print $2}'| xargs kill -9; rm -rf %s; git clone github.com/PaddlePaddle/%s.git; cd %s; python -m pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple''' % (pid, self.repo, self.repo, self.repo)
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
         output=repo_result[1]
         assert exit_code == 0, "git clone %s failed!   log information:%s" % (self.repo, output)
